...
module.exports = (function() {
  ...
  function runTests(id, input) {
    ...
    if (isCorrentLength(input) && !isValidISBN(input)) { invalidIsbn.write(id + ',' + input + '\n'); ++stats.invalidIsbn; }
    if (!isHyphenated(input)) { notHyphenated.write(id + ',' + input + '\n'); ++stats.notHyphenated; }
    if (containsInvalidChars(input)) { containsInvalidCharacters.write(id + ',' + input + '\n'); ++stats.containsInvalidCharacters; }
  }
